...
const handleResultPress = useCallback((result: SearchResult) => {
  ...
  switch (result.type) {
    case 'maintenance':
      router.push({ pathname: '/maintenance-detail', params: { id: result.id } });
      break;
    case 'issue':
...
<TouchableOpacity
  ...
  style={styles.maintenanceCard}
  activeOpacity={0.7}
  onPress={() => router.push({ pathname: '/maintenance-detail', params: { id: item.id } })}
>
  {/* ...existing children... */}
</TouchableOpacity>
```
...
<View key={task.id} style={[styles.taskCard, task.status === 'completed' && styles.taskCardCompleted]}>
  ...
  >
    {task.status === 'completed' ? (
      ...
      <IconSymbol ios_icon_name="circle" android_material_icon_name="radio-button-unchecked" size={28} color={colors.textSecondary} />
    )}
  </TouchableOpacity>
  <TouchableOpacity
...
<TouchableOpacity
  ...
  activeOpacity={0.7}
  onPress={() => router.push({ pathname: '/supply-detail', params: { id: request.id } })}
>
  {/* ...existing children... */}
</TouchableOpacity>
```
...
export default function VesselDetailScreen() {
  ...
  const { vessels, maintenanceTasks, issues, supplyRequests, documents } = useData();
        ...
        <View style={styles.header}>
          ...
          <View
            style={[
              ...
              {
                backgroundColor:
                  (vessel.status === 'active' ? colors.success : colors.warning) + '30',
              },
            ]}
          >
            <Text
          ...
          <View style={styles.section}>
            ...
            {pendingSupplies.slice(0, 5).map(req => (
              <TouchableOpacity
                ...
                activeOpacity={0.7}
                onPress={() =>
                  router.push({ pathname: '/supply-detail', params: { id: req.id } })
                }
              >
                <View style={styles.listCardContent}>
...
export function LinkedDetailRow({ label, value, linkTo }: LinkedDetailRowProps) {
  const content = (
    <View style={styles.row}>
      <Text style={styles.label}>{label}</Text>
      <View style={styles.valueRow}>
        <Text style={[styles.value, linkTo && styles.valueLinked]}>{value}</Text>
        {linkTo && (
          <IconSymbol
            ios_icon_name="chevron.right"
            android_material_icon_name="chevron-right"
